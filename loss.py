...
def compute_box_loss(gt: torch.Tensor, output: torch.Tensor, train_dataset: KittyDataset):
    B, _, M = output.shape
    ...
    for i in range(B):
        ...
        loss[i] = train_dataset.dist(center, size, car_prob, gt[i])
...
def compute_vote_loss(gt: torch.Tensor, xyz: torch.Tensor, vote_xyz: torch.Tensor, train_dataset: KittyDataset):
    ...
    for i in range(B):
        for k in range(len(gt[i])):
            ...
            m_pos = len(list)
            if m_pos != 0:
                loss[i] = loss[i].add(torch.sum(torch.abs(vote_xyz[i, list] - torch.from_numpy(gt_vote).to(device)), [0, 1]) / m_pos).to(device)
